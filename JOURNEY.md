# Ultimate Journey Protocol

This vault is strongest when followed in order.
Use this file as your navigation spine.

execution tracker:

- [Progress Tracker](./PROGRESS_TRACKER.md)

---

## Rules Of Progression

1. Do not skip stages.
2. Finish the listed notes in sequence.
3. Build while reading (commands, labs, scripts, notes).
4. Keep a weekly log of what you tested and broke.
5. Move to the next chapter only after completing the checkpoint.

---

# Phase 1

Phase 1 is about Operating System.
It is split into 3 chapters:
1. Shadow.Lab
2. SysAdmin
3. Bash (basic automation)

## Chapter 1: Shadow.Lab

Goal: build the lab, wire access, and work inside a controlled environment.

1. [Shadow.Lab](./Shadow.Lab/README.md)
2. [Linux Distros](./Shadow.Lab/1_distros.md)
3. [Installing Distro](./Shadow.Lab/2_distro_instalation.md)
4. [Arming Up](./Shadow.Lab/3_need_to_have.md)
5. [QEMU/KVM setup](./Shadow.Lab/4_QEMU_KVM.md)
6. [VM creation](./Shadow.Lab/5_controling_vms.md)

## Chapter 2: SysAdmin

Goal: become dangerous in Linux administration, not just comfortable in Linux.

1. [SysAdmin](./SysAdmin/README.md)
2. [CLI](./SysAdmin/1_cli.md)
3. [History](./SysAdmin/2_history.md)
4. [Root](./SysAdmin/3_root.md)
5. [File System](./SysAdmin/4_file_system.md)
6. [File Manipulation I](./SysAdmin/5_file_manipulation.md)
7. [File Manipulation II](./SysAdmin/6_file_manipulation.md)

## Chapter 3: Bash (Basic Automation)

Goal: automate operator work instead of repeating it by hand.

1. [Bash](./Scripting/Bash/README.md)
2. [Scripting](./Resources/Scripting.md)

---

## Deferred Material

Future topics and deferred notes live here:
- [Yeet](./Yeet/README.md)
